    ...
    public Salle addSalle(Salle salle) {
        try {
            Query query = entityManager.createNativeQuery(
                    "INSERT INTO salles (numero_salle, nom_salle, type_salle) " +
                            "VALUES (:numSalle, :nomSalle, CAST(:typeSalle AS type_salle_enum))"
            );
            query.setParameter("numSalle", salle.getNumSalle());
            query.setParameter("nomSalle", salle.getNomSalle());
            query.setParameter("typeSalle", salle.getTypeSalle().name()); // Utiliser name() pour passer une String
